...
public class AdminMenuRequestHandler extends AdminRequestHandler
{
	...
	protected void process()
	{
		...
		output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=userList&initial=true\">User Management</a>");
		output.println("</div>");
		
		output.println("<div class=\"adminMenuEntry\">");
		output.println("<span class=\"icon-font icon-watch iconAdminMenu\"></span>");
		output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=sessionList\">Active Sessions</a>");
		output.println("</div>");
		...
		if (WebFileSys.getInstance().getMailHost() !=null)
		{
			...
			output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=broadcast\">Broadcast e-mail</a>");
			output.println("</div>");
		}
		...
		if (WebFileSys.getInstance().isMaintananceMode())
		{
			...
			output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=switchMode\">Back to normal Operation</a>");
		}
		else
		{
			output.println("<span class=\"icon-font icon-tool iconAdminMenu\"></span>");
			output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=switchMode\">Switch to Maintanance Mode</a>");
		}
		output.println("</div>");

		output.println("<div class=\"adminMenuEntry\">");
		output.println("<span class=\"icon-font icon-hddrive iconAdminMenu\"></span>");
		output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=viewLog\">View Event Log</a>");
		output.println("</div>");

		output.println("<div class=\"adminMenuEntry\">");
		output.println("<span class=\"icon-font icon-exit iconAdminMenu\"></span>");
		output.println("<a href=\"/custom/webfilesys/servlet?command=admin&cmd=loginHistory\">Login/Logout Events</a>");
		output.println("</div>");

		output.println("<div style=\"padding-top:16px\">");
		output.println("<input type=\"button\" value=\"Logout\" onclick=\"window.location.href='/custom/webfilesys/servlet?command=logout'\">");
		
		output.println("<span style=\"width:32px;display:inline-block\"></span>");
		
		output.println("<input type=\"button\" value=\"Return\" onclick=\"window.location.href='/custom/webfilesys/servlet'\">");
		output.println("</div>");
