...
public class ProcessListRequestHandler extends UserRequestHandler
{
	...
	protected void process()
	{
		...
		output.println("    {window.location='/custom/webfilesys/servlet?command=killProcess&pid=' + pid;}");
		output.println("}");
			...
			else
			{
				...
				else
				{
					...
					if (pid >= 0)
					{
						...
						if (allowProcessKill && userMgr.getDocumentRoot(uid).equals("/"))
						{
							output.print("<td class=\"processKill\" ><a href=\"javascript:confirmKill('" + pid + "')\"><IMG ALIGN=\"center\" BORDER=\"0\" src=\"/custom/webfilesys/images/redx2.gif\" alt=\"kill process\"></a></td>");
						}
