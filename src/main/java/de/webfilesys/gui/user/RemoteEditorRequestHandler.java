...
public class RemoteEditorRequestHandler extends UserRequestHandler
{
	...
	protected void process()
	{
		...
		else
		{
			...
			output.println("<form id=\"editForm\" accept-charset=\"utf-8\" method=\"post\" action=\"/custom/webfilesys/servlet\" style=\"margin-top:20px\">");
			...
			if (mobile != null)
			{
                output.println("<input type=\"button\" value=\"" + getResource("button.cancel","Cancel") + "\" onclick=\"window.location.href='/custom/webfilesys/servlet?command=mobile&cmd=folderFileList';\">");
			} 
			else
			{
